...
public class EventService {
  ...
  public int heldSeats(String eventId) {
    return drafts.heldSeats(eventId);
  }
  ...
  public EventView create(Event in) {
    ...
    Event e =
        new Event(
            ...
            in.totalSeats(),
            in.price(),
            in.status() == null ? EventStatus.OPEN : in.status(),
            in.description());
    repo.save(e);
  ...
  public EventView update(String id, Event patch, long expectedRevision) {
    ...
    Event upd =
        new Event(
            ...
            patch.hall(),
            patch.totalSeats(),
            patch.totalSeats() - sold,
            patch.price(),
            patch.status() == null ? c.status() : patch.status(),
            patch.description());

    if (!repo.updateIfUnchanged(upd, expectedRevision))
      throw new ConflictException(
          "Событие изменено параллельно (ваша ревизия "
              + expectedRevision
              + ", текущая "
              + cur.modRevision()
