...
public class ViewCounterService {
  ...
  public long increment(String eventId) {
    return incrementKey(keys.eventViews(eventId));
  }

  public long get(String eventId) {
    return getKey(keys.eventViews(eventId));
  }
  ...
  private static void backoff(int attempt) {
    try {
      Thread.sleep(ThreadLocalRandom.current().nextLong(0, 1L + Math.min(attempt, 10)));
