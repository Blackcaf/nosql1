...
public class DraftService {
  ...
  public DraftView create(DraftRequest r, String managerLogin) {
    ...
    Event e =
        events
            .find(r.eventId())
            .orElseThrow(() -> new NotFoundException("Событие не найдено: " + r.eventId()))
            .value();
  ...
  public DraftView get(String id) {
    return drafts
        .find(id)
        .orElseThrow(() -> new NotFoundException("Временная заявка истекла или не найдена"));
  }

  public List<OrderDraft> all() {
    return drafts.findAll();
  }

  public long extend(String id) {
    return drafts.extend(id);
  }
