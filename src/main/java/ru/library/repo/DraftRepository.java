...
public class DraftRepository {
  ...
  public DraftRepository(KvTemplate kv, Keys keys, AppProperties props) {
    this.kv = kv;
    this.keys = keys;
    this.ttl = props.kv().draftTtlSeconds();
  }
  ...
  public DraftView create(OrderDraft d) {
    ...
    kv.store()
        .txn(
            List.of(),
            List.of(
                KvOp.put(keys.draft(d.id()), kv.toJson(d), leaseId),
  ...
  public Optional<DraftView> find(String id) {
    return findVersioned(id)
        .map(v -> new DraftView(v.value(), kv.store().leaseTimeToLive(v.leaseId())));
  }
  ...
  public long extend(String id) {
    Versioned<OrderDraft> v =
        findVersioned(id)
            .orElseThrow(
                () -> new NotFoundException("Временная заявка не найдена или истекла: " + id));
