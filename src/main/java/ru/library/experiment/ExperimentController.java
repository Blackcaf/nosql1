...
public class ExperimentController {
  ...
  public ExperimentController(
      ViewCounterService counter, OrderService orders, EventService events, KvTemplate kv) {
  ...
  @PostMapping("/orders")
  public OrdersResult orders(
      @RequestParam(defaultValue = "10") int seats, @RequestParam(defaultValue = "40") int attempts)
      throws Exception {
    EventView ev =
        events.create(
            new Event(
                null,
                "Стресс-тест " + LocalDateTime.now(),
                EventType.LECTURE,
      ...
      futures.add(
          pool.submit(
              () -> {
                ...
                try {
                  orders.placeOrder(
                      new OrderRequest(eventId, "Ч-" + n, "Читатель " + n, 1, null), "m" + n);
                  ok.incrementAndGet();
    ...
    long sold =
        orders.byEvent(eventId).stream()
            .filter(o -> o.status() == OrderStatus.CONFIRMED)
            .mapToInt(Order::seats)
            .sum();
