    ...
    synchronized (lock) {
      try {
        ...
        for (KvEntry e : s.entries()) {
          data.put(
              e.key(),
              new KvEntry(e.key(), e.value(), e.createRevision(), e.modRevision(), e.version(), 0));
        }
        log.info(
            "snapshot restored from {} ({} keys, revision {})", path, data.size(), revision.get());
      } catch (Exception e) {
  ...
  private void doPut(long rev, String key, String value, long leaseId) {
    ...
    KvEntry e =
        new KvEntry(
            key,
            value,
            old == null ? rev : old.createRevision(),
            rev,
  ...
  private void notify(WatchEvent.Type type, KvEntry entry, KvEntry previous) {
    for (Watcher w : watchers) {
      if (entry.key().startsWith(w.prefix())) {
        try {
          w.listener().accept(new WatchEvent(type, entry, previous));
        } catch (Exception ex) {
          log.warn("watch listener failed: {}", ex.toString());
        }
      }
    }
  }
